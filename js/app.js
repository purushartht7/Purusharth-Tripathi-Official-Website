...
import { addMessage, listenToMessages, getSettings, getProjects, getHighlights, getPosts, getReels, listenToVideos, getYouTubeId } from "./firebase-config.js";
...
async function initDynamicContent() {
    ...
    // C. Load Dynamic Highlights
    try {
        const highlights = await getHighlights();
        renderStoryHighlights(highlights);
    } catch (e) {
        console.error("Failed to load/render highlights:", e);
    }

    // D. Load Dynamic Posts
    try {
        const posts = await getPosts();
        renderPostsGrid(posts);
    } catch (e) {
        console.error("Failed to load/render posts:", e);
    }

    // E. Load Dynamic Reels
    try {
        const reels = await getReels();
        renderReelsGrid(reels);
    } catch (e) {
        console.error("Failed to load/render reels:", e);
    }
