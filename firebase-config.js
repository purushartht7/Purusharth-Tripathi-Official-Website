...
const mockHighlightsCallbacks = [];

function setupMockHighlightsListener(callback) {
    mockHighlightsCallbacks.push(callback);
    const local = localStorage.getItem("pt_highlights");
    if (!local) {
        localStorage.setItem("pt_highlights", JSON.stringify(defaultHighlights));
    }
    const items = JSON.parse(localStorage.getItem("pt_highlights") || "[]");
    callback(items.map(hl => migrateHighlight(hl)));
}

function triggerMockHighlightsChange() {
    const items = JSON.parse(localStorage.getItem("pt_highlights") || "[]");
    mockHighlightsCallbacks.forEach(cb => cb(items.map(hl => migrateHighlight(hl))));
}

export async function listenToHighlights(callback) {
    if (useFirebase && db) {
        try {
            const { collection, onSnapshot, query, orderBy } = await import("https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js");
            ...
            return onSnapshot(q, (snapshot) => {
                ...
                console.error("Firestore highlights snapshot error, falling back to Mock:", error);
                setupMockHighlightsListener(callback);
            });
        } catch (e) {
            console.error("Failed to setup Firestore highlights listener, using Mock:", e);
        }
    }

    setupMockHighlightsListener(callback);
    return () => {
        const index = mockHighlightsCallbacks.indexOf(callback);
        if (index > -1) mockHighlightsCallbacks.splice(index, 1);
    };
}
...
const mockPostsCallbacks = [];

function setupMockPostsListener(callback) {
    mockPostsCallbacks.push(callback);
    const local = localStorage.getItem("pt_posts");
    if (!local) {
        localStorage.setItem("pt_posts", JSON.stringify(defaultPosts));
    }
    const items = JSON.parse(localStorage.getItem("pt_posts") || "[]");
    callback(items);
}

function triggerMockPostsChange() {
    const items = JSON.parse(localStorage.getItem("pt_posts") || "[]");
    mockPostsCallbacks.forEach(cb => cb(items));
}

export async function listenToPosts(callback) {
    if (useFirebase && db) {
        try {
            const { collection, onSnapshot, query, orderBy } = await import("https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js");
            ...
            return onSnapshot(q, (snapshot) => {
                ...
                console.error("Firestore posts snapshot error, falling back to Mock:", error);
                setupMockPostsListener(callback);
            });
        } catch (e) {
            console.error("Failed to setup Firestore posts listener, using Mock:", e);
        }
    }

    setupMockPostsListener(callback);
    return () => {
        const index = mockPostsCallbacks.indexOf(callback);
        if (index > -1) mockPostsCallbacks.splice(index, 1);
    };
}
...
const mockReelsCallbacks = [];

function setupMockReelsListener(callback) {
    mockReelsCallbacks.push(callback);
    const local = localStorage.getItem("pt_reels");
    if (!local) {
        localStorage.setItem("pt_reels", JSON.stringify(defaultReels));
    }
    const items = JSON.parse(localStorage.getItem("pt_reels") || "[]");
    callback(items);
}

function triggerMockReelsChange() {
    const items = JSON.parse(localStorage.getItem("pt_reels") || "[]");
    mockReelsCallbacks.forEach(cb => cb(items));
}

export async function listenToReels(callback) {
    if (useFirebase && db) {
        try {
            const { collection, onSnapshot, query, orderBy } = await import("https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js");
            ...
            return onSnapshot(q, (snapshot) => {
                ...
                console.error("Firestore reels snapshot error, falling back to Mock:", error);
                setupMockReelsListener(callback);
            });
        } catch (e) {
            console.error("Failed to setup Firestore reels listener, using Mock:", e);
        }
    }

    setupMockReelsListener(callback);
    return () => {
        const index = mockReelsCallbacks.indexOf(callback);
        if (index > -1) mockReelsCallbacks.splice(index, 1);
    };
}
